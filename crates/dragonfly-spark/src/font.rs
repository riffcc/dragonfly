...
//!
//! Simple 8x16 bitmap font for text display
...
pub fn draw_char(x: u32, y: u32, c: char, color: u32) {
    ...
    let glyph = &FONT_8X16[idx * 16..idx * 16 + 16];
...
pub fn draw_string(x: u32, y: u32, s: &str, color: u32) {
    ...
    for c in s.chars() {
        if c == '\n' {
            // Don't handle newlines here, caller should manage
            continue;
        }
...
/// Draw a string with a background color
pub fn draw_string_bg(x: u32, y: u32, s: &str, fg: u32, bg: u32) {
    // Draw background first
    let width = s.len() as u32 * CHAR_WIDTH;
    framebuffer::fill_rect(x, y, width, CHAR_HEIGHT, bg);

    // Then text
    draw_string(x, y, s, fg);
}

/// Draw centered text
pub fn draw_string_centered(y: u32, s: &str, color: u32, screen_width: u32) {
    let text_width = s.len() as u32 * CHAR_WIDTH;
    let x = (screen_width - text_width) / 2;
    draw_string(x, y, s, color);
}

/// Draw large text (2x scale)
pub fn draw_string_large(x: u32, y: u32, s: &str, color: u32) {
    let mut cx = x;
    for c in s.chars() {
        draw_char_scaled(cx, y, c, color, 2);
        cx += CHAR_WIDTH * 2;
    }
}

/// Draw a character at a specified scale
pub fn draw_char_scaled(x: u32, y: u32, c: char, color: u32, scale: u32) {
    let idx = c as usize;
    if idx >= 128 {
        return;
    }
...
/// 8x16 bitmap font - standard PC BIOS font
