...
impl RqliteStore {
    ...
    fn dns_record_from_rqlite_row(row: &rqlite_rs::Row) -> Result<DnsRecord> {
        ...
        let name: String = row
            .get("name")
            .map_err(|e| StoreError::Database(format!("column error: {}", e)))?;
        let rtype_str: String = row
            .get("rtype")
            .map_err(|e| StoreError::Database(format!("column error: {}", e)))?;
        let rdata: String = row
            .get("rdata")
            .map_err(|e| StoreError::Database(format!("column error: {}", e)))?;
        let ttl: i64 = row
            .get("ttl")
        ...
        let source_str: String = row
            .get("source")
            .map_err(|e| StoreError::Database(format!("column error: {}", e)))?;
        let machine_id_str: String = row.get("machine_id").unwrap_or_default();
...
impl Store for RqliteStore {
    ...
    async fn get_machine(&self, id: Uuid) -> Result<Option<Machine>> {
        let q = rqlite_rs::query!("SELECT data FROM machines WHERE id = ?", id.to_string());
    ...
    async fn put_machine(&self, machine: &Machine) -> Result<()> {
        ...
        let old_rows = self
            ...
            .await
            .map_err(|e| StoreError::Database(format!("rqlite query error: {}", e)))?;
        let old_tags: Vec<String> = old_rows
            .iter()
            .filter_map(|r| r.get::<String>("tag").ok())
            .collect();

        // Delete old tags
        let del = rqlite_rs::query!("DELETE FROM machine_tags WHERE machine_id = ?", id.clone());
        self.client
            .exec(del)
            .await
            .map_err(|e| StoreError::Database(format!("rqlite exec error: {}", e)))?;
        ...
        for old_tag in &old_tags {
            ...
            let count: i64 = rows
                ...
                .unwrap_or(0);
            if count == 0 {
                let del_tag = rqlite_rs::query!("DELETE FROM tags WHERE name = ?", old_tag.clone());
                self.client
                    .exec(del_tag)
                    .await
                    .map_err(|e| StoreError::Database(format!("rqlite exec error: {}", e)))?;
    ...
    async fn delete_machine(&self, id: Uuid) -> Result<bool> {
        ...
        let del_machine = rqlite_rs::query!("DELETE FROM machines WHERE id = ?", id_str);
        let result = self
            .client
            .exec(del_machine)
            .await
        ...
        for tag in &tags {
            ...
            let count: i64 = rows
                ...
                .unwrap_or(0);
            if count == 0 {
                let del_tag = rqlite_rs::query!("DELETE FROM tags WHERE name = ?", tag.clone());
                self.client
                    .exec(del_tag)
                    .await
                    .map_err(|e| StoreError::Database(format!("rqlite exec error: {}", e)))?;
    ...
    async fn list_all_tags(&self) -> Result<Vec<String>> {
        let rows = self
            .client
            .fetch("SELECT name FROM tags ORDER BY name")
            .await
            .map_err(|e| StoreError::Database(format!("rqlite query error: {}", e)))?;
        Ok(rows
            .iter()
    ...
    async fn delete_tag(&self, tag: &str) -> Result<bool> {
        ...
        let del_machine_tags =
            rqlite_rs::query!("DELETE FROM machine_tags WHERE tag = ?", tag.to_string());
        let r2 = self
            .client
            .exec(del_machine_tags)
            .await
            .map_err(|e| StoreError::Database(format!("rqlite exec error: {}", e)))?;
        ...
        if r2.changed() {
            ...
            let rows = self
                .client
                .fetch("SELECT id, data FROM machines")
                .await
                .map_err(|e| StoreError::Database(format!("rqlite query error: {}", e)))?;
                ...
                if let Ok(mut machine) = Self::from_json::<Machine>(&json) {
                    if machine.config.tags.contains(&tag.to_string()) {
                        ...
                        if let Ok(updated_json) = Self::to_json(&machine) {
                            ...
                            self.client.exec(upd).await.map_err(|e| {
                                StoreError::Database(format!("rqlite exec error: {}", e))
                            })?;
                        }
                    }
                }
    ...
    async fn get_template(&self, name: &str) -> Result<Option<Template>> {
        let q = rqlite_rs::query!(
            "SELECT data FROM templates WHERE name = ?",
            name.to_string()
        );
        match self.fetch_one_data(q).await? {
            Some(json) => Ok(Some(Self::from_json(&json)?)),
    ...
    async fn get_workflows_for_machine(&self, machine_id: Uuid) -> Result<Vec<Workflow>> {
        let q = rqlite_rs::query!(
            "SELECT data FROM workflows WHERE machine_id = ?",
    ...
    async fn get_setting(&self, key: &str) -> Result<Option<String>> {
        ...
        match rows.first() {
            Some(row) => {
                Ok(Some(row.get::<String>("value").map_err(|e| {
                    StoreError::Database(format!("column error: {}", e))
                })?))
            }
            None => Ok(None),
        }
    ...
    async fn list_users(&self) -> Result<Vec<User>> {
        let jsons = self
            .fetch_all_data("SELECT data FROM users ORDER BY json_extract(data, '$.created_at')")
            .await?;
        jsons.iter().map(|j| Self::from_json(j)).collect()
    }
    ...
    async fn get_api_token_by_hash(&self, token_hash: &str) -> Result<Option<ApiToken>> {
        let q = rqlite_rs::query!(
            "SELECT data FROM api_tokens WHERE token_hash = ?",
    ...
    async fn list_api_tokens(&self) -> Result<Vec<ApiToken>> {
        let jsons = self
            .fetch_all_data(
                "SELECT data FROM api_tokens ORDER BY json_extract(data, '$.created_at')",
            )
            .await?;
        jsons.iter().map(|j| Self::from_json(j)).collect()
    }
    ...
    async fn delete_dns_records_by_machine(&self, machine_id: Uuid) -> Result<u64> {
        let q = rqlite_rs::query!(
            "DELETE FROM dns_records WHERE machine_id = ?",
            machine_id.to_string()
        );
        let result = self
            .client
