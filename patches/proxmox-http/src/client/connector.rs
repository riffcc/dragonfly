...
// macOS stub for set_tcp_keepalive - uses standard socket options
#[cfg(not(target_os = "linux"))]
fn set_tcp_keepalive<S: std::os::fd::AsFd>(stream: &S, time: u32) -> nix::Result<()> {
    use nix::sys::socket::{setsockopt, sockopt::KeepAlive};
    setsockopt(stream, KeepAlive, &true)?;
    // Note: TCP_KEEPIDLE equivalent on macOS is TCP_KEEPALIVE but requires more complex handling
    // For now, just enable keepalive - the time parameter is best-effort
    let _ = time;
    Ok(())
}
