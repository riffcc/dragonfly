...
pub enum TlsOptions {
    ...
    /// Expect a specific certificate fingerprint (SHA-256).
    Fingerprint(Vec<u8>),
}
...
impl Client {
    ...
    pub fn with_options(
        ...
        let tls_config = match tls_options {
            ...
            TlsOptions::Insecure => ClientConfig::builder()
                .dangerous()
                .with_custom_certificate_verifier(Arc::new(InsecureVerifier))
                .with_no_client_auth(),
            TlsOptions::Fingerprint(expected_fingerprint) => ClientConfig::builder()
                .dangerous()
        ...
        let client =
            proxmox_http::client::Client::with_tls_config(Arc::new(tls_config), http_options);
    ...
    /// Check to see if we need to refresh the ticket.
    ...
    /// If the ticket expires soon, this will attempt to refresh the ticket.
...
// --- Custom TLS verifiers for rustls ---

/// Accepts any certificate without verification (insecure mode).
#[derive(Debug)]
...
/// Verifies that the server certificate matches a specific SHA-256 fingerprint.
#[derive(Debug)]
...
impl ServerCertVerifier for FingerprintVerifier {
    fn verify_server_cert(
        ...
        if fingerprint.as_slice() == self.expected.as_slice() {
            ...
            log::error!("bad fingerprint: {}", fp_string(&fingerprint));
            log::error!("expected fingerprint: {}", fp_string(&self.expected));
