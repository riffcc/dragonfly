...
impl ProcessLocker {
    ...
    fn try_lock(file: &std::fs::File, ltype: i16) -> Result<(), Error> {
        let op = libc::flock {
            l_type: ltype,
            l_whence: libc::SEEK_SET as i16,
            l_start: 0,
    ...
    pub fn try_shared_lock(locker: Arc<Mutex<Self>>) -> Result<ProcessLockSharedGuard, Error> {
        ...
        if data.writers == 0 && !data.exclusive {
            if let Err(err) = Self::try_lock(&data.file, libc::F_RDLCK as i16) {
    ...
    pub fn try_exclusive_lock(
        ...
        if let Err(err) = Self::try_lock(&data.file, libc::F_WRLCK as i16) {
