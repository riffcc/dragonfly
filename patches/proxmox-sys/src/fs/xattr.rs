...
/// Return a list of extended attributes accessible as an iterator over items of type `&CStr`.
/// macOS version with extra options argument
#[cfg(not(target_os = "linux"))]
pub fn flistxattr(fd: RawFd) -> Result<ListXAttr, nix::errno::Errno> {
    let mut size = 256;
    let mut buffer = vec::undefined(size);
    let mut bytes =
        unsafe { libc::flistxattr(fd, buffer.as_mut_ptr() as *mut libc::c_char, buffer.len(), 0) };
    while bytes < 0 {
        let err = Errno::last();
        match err {
            Errno::ERANGE => {
...
/// Get an extended attribute by name (Linux version).
#[cfg(target_os = "linux")]
...
/// Get an extended attribute by name (macOS version with position and options).
#[cfg(not(target_os = "linux"))]
pub fn fgetxattr(fd: RawFd, name: &CStr) -> Result<Vec<u8>, nix::errno::Errno> {
    let mut size = 256;
    let mut buffer = vec::undefined(size);
...
/// Set an extended attribute on a file descriptor (Linux version).
#[cfg(target_os = "linux")]
...
/// Set an extended attribute on a file descriptor (macOS version with position and options).
#[cfg(not(target_os = "linux"))]
pub fn fsetxattr(fd: RawFd, name: &CStr, data: &[u8]) -> Result<(), nix::errno::Errno> {
    let result = unsafe {
        libc::fsetxattr(
